...
png('environment_covid_20221210.png', width=460, height=230)
...
png('covid_coefficient_20221210.png', width=460, height=230)
par(mfrow=c(2,4), mar=c(4,2,2,1.5), ps=16, font.main=16)
for (i in 1:7) {
  ind = order(u)
  lower = min(B1[,i], B2[,i]) - 0.2
  upper = max(B1[,i], B2[,i]) + 0.2
  plot(t[ind], B1[ind, i], type='n', xlab='time', ylab=paste0('B', i), ylim=c(lower, upper), main=titles[i])
  grid(6, NA, col='grey')
  lines(t[ind], B1[ind, i])
  lines(t[ind], B2[ind, i], lty=2)
}
dev.off()
...
png('lag_forward_20221210.png', width=460, height=230)
par(mfrow=c(1,1), mar=c(4,2,2,1.5), ps=16, font.main=16)
